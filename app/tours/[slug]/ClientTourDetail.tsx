...
import { 
  ArrowLeft, Star, Clock, Map, Share, Heart, CheckCircle, 
  MapPin, Utensils, Bus, Bed, CheckCircle2, Plus, Flag, FileText 
} from "lucide-react"; // Hash (Trip Code) අයින් කළා

export default function ClientTourDetail({ tour }: { tour: any }) {
            ...
            <div className="mb-10 flex flex-col gap-3">
              ...
              {tour.gallery && tour.gallery.length > 0 && (
                  <div className="grid grid-cols-4 gap-3 h-24 md:h-32">
                      {tour.gallery.slice(0, 4).map((img: any, idx: number) => {
                          const isLastVisible = idx === 3;
                          const totalPhotos = (tour.gallery?.length || 0) + 1;
                          const showOverlay = isLastVisible && tour.gallery.length >= 4;
                          
                          const imgSrc = typeof img === 'string' ? img : img.url;
                          const imgAlt = typeof img === 'object' && img.alt ? img.alt : `${tour.title} Gallery ${idx + 1}`;
          ...
          <div className="w-full lg:w-1/3 relative">
            <div className="sticky top-28 bg-white border border-gray-200 rounded-2xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
              ...
              <div className="space-y-6 border-y border-gray-200 py-6 mb-6">
                ...
                {/* අලුතින් එකතු කළ Route Map Image එක සහ Route Name එක */}
                <div className="space-y-3">
                    {tour.routeImage && (
                        <div className="w-full h-32 bg-gray-100 rounded-xl overflow-hidden border border-gray-200">
                            <img src={tour.routeImage} alt={`${tour.title} Route Map`} className="w-full h-full object-cover" />
                        </div>
                    )}
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <span className="block font-bold text-gray-900">{tour.route || "Not specified"}</span>
                        </div>
                    </div>
                </div>
              </div>
